...
## place cursor
          ...
          return
## SQL statement
     cursor.execute ("SELECT DISTINCT make, model, price, overallRating FROM car_ratings INNER JOIN car_details ON car_ratings.carID=car_details.carID ORDER BY price desc, make asc, model asc, overallRating desc LIMIT %s",number)
## Get results
     values = cursor.fetchall()
## Disconnect from db
     connection.close()    
##convert results
     for value in values:
          print str(value[0])+' '+str(value[1])+': '+'$'+str(value[2])+', '+'Overall rating'+' '+str(value[3])  
...
## place cursor
          ...
          return
## SQL statement
     cursor.execute("SELECT model, avg(overallRating) as Average FROM car_ratings INNER JOIN car_details ON car_ratings.carID=car_details.carID WHERE make='"+make+"' GROUP BY model ORDER BY Average desc, model asc")
## Get results
     values = cursor.fetchall()
## Disconnect from db
     connection.close()
